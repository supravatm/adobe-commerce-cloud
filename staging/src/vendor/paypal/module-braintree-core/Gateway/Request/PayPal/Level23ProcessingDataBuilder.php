...
class Level23ProcessingDataBuilder implements BuilderInterface
{
    ...
    public function build(array $buildSubject): array
    {
        ...
        if ($ppSendCartLineItems && $isPayPalMethod) {
            foreach ($order->getItems() as $item) {
                ...
                if ($item->getParentItem() || 0.0 === (float) $item->getBasePrice()) {
                    continue;
                }
        ...
        if ($order->getShippingAddress()) {
            ...
            // Level 3.
            $processingData[self::KEY_SHIPPING_AMT] = $this->getShippingAmount($order);
    ...
    private function getTaxAmount(OrderAdapter $order): string
    {
        ...
        if ($order->getBaseShippingTaxAmount()) {
     ...
     * @return string
     */
    private function getShippingAmount(OrderAdapter $order): string
    {
        $shippingAmount = $order->getBaseShippingAmount();
        if ($order->getBaseShippingTaxAmount()) {
            $shippingAmount += $order->getBaseShippingTaxAmount();
        }
