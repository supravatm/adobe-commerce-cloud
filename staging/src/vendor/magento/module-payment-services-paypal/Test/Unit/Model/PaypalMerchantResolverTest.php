...
class PaypalMerchantResolverTest extends TestCase
{
     ...
     * ]
     */
    public static function paypalMerchantProviderForMBAAtWebsiteLevel(): array
    {
        return [
            'Global Scope' => [
                PaypalMerchantResolver::GLOBAL_SCOPE,
                0,
                'paypalsellerid_global',
                'COMPLETED',
                null
            ],
            'Website Scope' => [
                ...
                1,
                'paypalsellerid_website1',
                'STARTED',
                null
            ],
            'Store View Scope with config at website level' => [
                ScopeHeadersBuilder::STOREVIEW_SCOPE_TYPE,
                2,
                'paypalsellerid_website1',
                'STARTED',
                1
            ],
            'Store View Scope without config at website level' => [
                ScopeHeadersBuilder::STOREVIEW_SCOPE_TYPE,
                3,
                'paypalsellerid_global',
                'COMPLETED',
     ...
     * @param int $scopeId
     * @param string $expectedMerchantId
     * @param string|null $expectedMerchantStatus
     * @return void
     */
     ...
     * ]
     */
    public static function paypalMerchantProviderForMBAAtStoreViewLevel(): array
    {
        return [
            'Global Scope' => [
                PaypalMerchantResolver::GLOBAL_SCOPE,
            ...
            'Store View Scope with config' => [
                ScopeHeadersBuilder::STOREVIEW_SCOPE_TYPE,
                2,
                'paypalsellerid_storeview2',
                null
            ],
            'Store View Scope without config' => [
                ScopeHeadersBuilder::STOREVIEW_SCOPE_TYPE,
                3,
                'paypalsellerid_global',
                'COMPLETED'
     ...
     * @return void
     */
    public function testGetExactPayPalMerchantReturnsExactWebsiteScopeMerchant(): void
    {
        $this->merchantService->expects($this->once())
            ->method('getAllScopesForMerchant')
            ->with('sandbox')
            ->willReturn($this->getScopes());

        $paypalMerchant = $this->paypalMerchantResolver->getExactPayPalMerchant(
            ScopeHeadersBuilder::WEBSITE_SCOPE_TYPE,
            1,
            'sandbox'
        );

        $this->assertNotNull($paypalMerchant);
        $this->assertEquals('paypalsellerid_website1', $paypalMerchant->getId());
        $this->assertEquals('STARTED', $paypalMerchant->getStatus());
    }
     ...
     * @return void
     */
    public function testGetExactPayPalMerchantReturnsNullWithoutGlobalFallback(): void
    {
        $this->merchantService->expects($this->once())
            ->method('getAllScopesForMerchant')
            ->with('sandbox')
        ...
        $this->assertNull(
            $this->paypalMerchantResolver->getExactPayPalMerchant(
                ScopeHeadersBuilder::WEBSITE_SCOPE_TYPE,
                999,
                'sandbox'
            )
     ...
     * @return void
     */
    public function testGetPayPalMerchantPassesEnvironmentToMerchantService(): void
    {
        $this->config->expects($this->once())
            ->method('getMultiBusinessAccountScopingLevel')
            ->willReturn(ScopeInterface::SCOPE_WEBSITE);
        ...
        $paypalMerchant = $this->paypalMerchantResolver->getPayPalMerchant(
            ScopeHeadersBuilder::WEBSITE_SCOPE_TYPE,
            1
        );
     ...
     * @return void
     */
    public function testGetExactPayPalMerchantPassesEnvironmentToMerchantService(): void
    {
        $this->merchantService->expects($this->once())
            ->method('getAllScopesForMerchant')
            ->with('sandbox')
